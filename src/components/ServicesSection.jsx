              ...
              >
                ...
                <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-white/40 font-mono group-hover:text-white/70 transition-colors flex items-center gap-1">
                    View Details <ArrowUpRight className="w-3 h-3 inline" />
                  </span>
                </div>
              </motion.div>
            );
          ...
          {selectedArenaModal && (
            ...
            >
              ...
              >
                ...
                {/* Brochure Image Display Area with Scroll Support for Additional Themes */}
                <div className="flex-1 overflow-y-auto p-3 sm:p-5 bg-black/40 flex flex-col items-center gap-5">
                  {selectedArenaModal.brochure ? (
                    <div className="w-full flex flex-col items-center gap-2">
                      <div className="relative w-full flex items-center justify-center rounded-2xl border border-white/10 bg-neutral-900/50 shadow-inner p-1 sm:p-2">
                        <img
                          src={selectedArenaModal.brochure}
                          alt={`${selectedArenaModal.name} Event Brochure`}
                          className="w-auto h-auto max-h-[70vh] max-w-full object-contain rounded-xl shadow-2xl"
                          loading="eager"
                        />
                      </div>

                      {selectedArenaModal.themesImage && (
                        <div className="flex items-center gap-1.5 text-emerald-400 font-mono text-[11px] sm:text-xs pt-1 animate-pulse">
                          <span>↓ Scroll down to view the 7 Presentation Themes & Domains</span>
                        </div>
                      )}
                    </div>
                  ) : (
                  ...
                  {/* Additional Themes Image (e.g. PitchUp Problem Statements / Domains) */}
                  {selectedArenaModal.themesImage && (
                    <div className="w-full flex flex-col items-center gap-3 pt-4 border-t border-white/15">
                      <div className="w-full flex items-center justify-between gap-2 px-1 flex-wrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs sm:text-sm font-mono font-bold uppercase tracking-wider text-emerald-400">
                            Official Presentation Themes & Domains
                          </span>
                        </div>
                        <span className="text-[10px] sm:text-xs font-mono text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 px-2.5 py-0.5 rounded-full">
                          7 Tracks
                        </span>
                      </div>

                      <div className="relative w-full flex items-center justify-center rounded-2xl border border-emerald-500/30 bg-neutral-900/50 shadow-2xl p-1 sm:p-2">
                        <img
                          src={selectedArenaModal.themesImage}
                          alt={`${selectedArenaModal.name} Official Presentation Themes & Domains`}
                          className="w-auto h-auto max-w-full object-contain rounded-xl shadow-2xl"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto">
                    {selectedArenaModal.brochure && (
                      <a
                        ...
                        className="text-xs font-mono text-white/80 hover:text-white flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition-all"
                      >
                    ...
                    {selectedArenaModal.themesImage && (
                      <a
                        href={selectedArenaModal.themesImage}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-xs font-mono text-emerald-300 hover:text-white flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 transition-all"
                      >
                        <ExternalLink className="w-3.5 h-3.5" />
                        <span>Open Themes Chart</span>
                      </a>
                    )}
                  </div>
